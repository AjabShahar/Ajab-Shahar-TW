(function () {
    ...
    (function (a, b, c) {
        ...
        function F(a) {
            function f(a) {
                ...
                if (A.length > 1)for (var d = 0; d < A.length; ++d) {
                    ...
                    if (f.disable && f.disable(w, x, u, v)) {
                        ...
                        if (A.length == 1) {
                            var h = A[0];
                            T("scrollability-lock", h.node, {direction: h.direction})
                        }
                    }
                }
            ...
            var b = g ? a.touches[0] : a, c = null;
            w = u = b.clientX, x = v = b.clientY, y = !1, A = H(a.target, w, x, a.timeStamp);
            if (!A.length)return !0;
            var d = a.target, e = setTimeout(function () {
                e = 0, c = L(d)
            }, 50);
        ...
        function G(a, b, c, d) {
            function X(a, b) {
                N.push({node: a, callback: b, keyframes: []})
            }
            ...
            function _(a) {
                var b = -w - x;
                if (u && j < b) {
                    var c = j - q * 2, d = c / b * c, e;
                    if (a > x)d = Math.max(d - (a - x), r), e = 0; else if (a < w) {
                        var f = d - (w - a);
            ...
            function bb() {
                function Q(a) {
                    var f = v - c;
                    if (a || b - d >= t || e < 0 != f < 0)o.push({position: v, time: b}), N.forEach(function (a) {
                        a.keyframes.push({time: b, css: a.callback(v)})
                    }), e = f, c = v, d = b
                }

                var b = 0, c = v, d = 0, e = 0, i, k, m = 0, n, o = [];
                if (g)if (Math.abs(v - x) > H || Math.abs(E) > p)if (v > x) {
                    if (x != A) {
                        x += j + D, w += j + D;
                        var q = w % j, r = -Math.round((v + j - q) / (j + D));
                        T("scrollability-page", a.node, {page: r})
                    }
                } else if (w != z) {
                    x -= j + D, w -= j + D;
                    var q = w % j, r = -Math.round((v - j - q) / (j + D));
                ...
                while (u) {
                    ...
                    } else m || (i = v, k = x - v), v = O(m, i, k, F), u = ++m <= F && Math.floor(Math.abs(v)) > x; else if (v < w && f)if (E < 0) {
                        var y = w - v, C = 1 - y / G;
                        E = Math.min(E + l, 0) * C, v += E
                    } else m || (i = v, k = w - v), v = O(m, i, k, F), u = ++m <= F && Math.ceil(v) < w; else {
                ...
                var J = o.length ? o[o.length - 1].time : 0, L = "scrollability" + B++, M = K(a, o, L, J, P);
                return {time: J, position: v, keyframes: o, name: L, css: M}
            }

            function bc(b) {
                if (!T("scrollability-scroll", e, {direction: a.direction, position: b}))return;
                bd(b), u && y && S(u)
            }
            ...
            var e = a.node, f = a.constrained, g = a.paginated, j = a.viewport || 0, u = a.scrollbar, v = a.position, w = a.min, x = a.max, z = w, A = Math.round(x / j) * j, D = 0, E = 0, F = g ? n : m, G = a.bounce, H = j * o, I = startTouch = a.filter(b, c), J = d, L = 0, M = 0, N = [], P = e.scrollableOffset || 0;
            if (!a.mute) {
                var Q = {position: v, track: X, setSpacing: Y, setOffset: Z};
                if (!T("scrollability-start", e, Q))return null
            }
            if (g) {
                if (D === undefined) {
                    var U = Math.round(Math.abs(z) % j), V = (Math.abs(z) - U) / j + 1;
                    D = U / V
                }
        ...
        function H(a, b, c, d) {
            var e = [], f = document.querySelectorAll(".scrollable.universal");
        ...
        function K(a, b, c, d, e) {
            ...
            return b.forEach(function (b) {
                var c = b.time / d * 100, g = Math.floor(c) + "% {" + "-webkit-transform: " + (b.css || a.update(b.position + e)) + ";" + "}";
                f.push(g)
            }), f.push("}"), f.join("\n")
        }
        ...
        function P(a) {
            var b = a.parentNode, c = a.querySelector(".scrollable > .clipper") || a;
            a.style.webkitAnimation && (a.style.webkitAnimationPlayState = "paused");
            var d = getComputedStyle(a).webkitTransform, e = (new WebKitCSSMatrix(d)).m41 - (a.scrollableOffset || 0);
            return {
                node: a,
                ...
                disable: function (a, b, c, d) {
                    var e = Math.abs(a - c), f = Math.abs(b - d);
                    if (f > e && f > j)return !0
                },
                update: function (a) {
        ...
        function Q(a) {
            var b = a.parentNode, c = a.querySelector(".scrollable > .clipper") || a;
            a.style.webkitAnimation && (a.style.webkitAnimationPlayState = "paused");
            var d = getComputedStyle(a).webkitTransform, e = (new WebKitCSSMatrix(d)).m42;
            return {
                node: a,
                scrollbar: N(a),
                position: e,
        ...
        var e = "webkitTransform"in document.documentElement.style, f = e && /OS 5_/.exec(navigator.userAgent), g = "ontouchstart"in window, h = .9925, i = 4, j = 10, k = .75, l = .01, m = 240, n = 160, o = .5, p = 2, q = 2, r = 7, s = 4, t = 24, u, v, w, x, y, z = 0, A = [], B = 0, C, D = {
            horizontal: P,
            vertical: Q
        };
        b.directions = D, b.flashIndicators = function () {
        }, a.ready(function () {
